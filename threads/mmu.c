	...
	for (unsigned i = 0; i < PGSIZE / sizeof(uint64_t *); i++) {
		...
		if (((uint64_t)*pte) & PTE_P) {
			...
			if (!func(pte, va, aux))
				return false;
			// if (is_user_vaddr(va))
				// printf("va: %p, pml4_index: %p, pdp_index: %p, pdx_index: %p, i: %p\n", va, pml4_index << PML4SHIFT, pdp_index << PDPESHIFT, pdx_index << PDXSHIFT, i << PTXSHIFT);
		}
	}
	return true;
}
...
pml4_get_page(uint64_t *pml4, const void *uaddr) {
	...
	if (pte && (*pte & PTE_P)) {
		// printf("%p\n", ptov(PTE_ADDR(*pte)) + pg_ofs(uaddr));
		return ptov(PTE_ADDR(*pte)) + pg_ofs(uaddr); // PTE address+page offset
	}
	return NULL;
}
