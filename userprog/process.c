...
duplicate_pte (uint64_t *pte, void *va, void *aux) {
	...
	// newpage = palloc_get_multiple (PAL_USER, FDT_PAGES);
	if (newpage == NULL)
		return false; 
	...
	// memcpy(newpage, parent_page, PGSIZE*FDT_PAGES);
...
process_exec (void *f_name) {
	...
	// palloc_free_multiple (file_name, FDT_PAGES);
	if (!success)
		return -1;
...
process_exit (void) {
	...
	// palloc_free_multiple (table, FDT_PAGES);
